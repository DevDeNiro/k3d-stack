...
if [ "$create_new_cluster" = true ]; then
    ...
    if [ -f kube/k3d-config.yaml ]; then
        echo -e "${YELLOW}Using configuration file: kube/k3d-config.yaml${NC}"
        # Try with config file first
        if ! k3d cluster create --config kube/k3d-config.yaml; then
...
if [ "$install_vault" = true ]; then
    ...
    helm install vault hashicorp/vault --namespace vault --create-namespace \
        -f helm/vault-values.yaml --wait --timeout 120s
...
if [ "$install_prometheus" = true ]; then
    ...
    helm install prometheus prometheus-community/prometheus --namespace monitoring --create-namespace \
        -f helm/prometheus-values.yaml --wait --timeout 120s
...
if [ "$install_minio" = true ]; then
    ...
    helm install minio minio/minio --namespace storage --create-namespace \
        ...
        -f helm/minio-values.yaml --wait --timeout 90s
...
if [ "$install_dashboard" = true ]; then
    ...
    kubectl apply -f kube/admin-token-secret.yaml
